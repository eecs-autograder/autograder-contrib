...
from ag_contrib.config.models import BUILTIN_FDBK_PRESETS, AGConfig, AGTestCommandFeedbackConfig, CommandConfig, InstructorFileConfig, ProjectConfig, TestCaseConfig, TestSuiteConfig
...
def save_project(config_file: str, *, base_url: str, token_file: str):
    ...
    course_response = client.get(f'/api/course/{course_data.name}/{course_data.semester.value}/{course_data.year}/')
    ...
    if project is None:
        ...
        response = client.post(f'/api/courses/{course["pk"]}/projects/', json={'name': config.project.settings.name})
    ...
    for suite_data in config.project.test_suites:
        ...
        if suite_data.name in test_suites:
            response = client.patch(
                ...
                json=suite_data.model_dump(exclude_unset=True, exclude=['test_cases', 'student_files_needed', 'instructor_files_needed'])
                     | {'student_files_needed': [student_files[pattern] for pattern in suite_data.student_files_needed],
                        'instructor_files_needed': [instructor_files[name] for name in suite_data.instructor_files_needed]}
            )
            check_response_status(response)
...
def _create_or_update_test_shallow(
    ...
    *,
    ...
    if test_data.name in test_cases:
        response = client.patch(
            f'/api/ag_test_cases/{test_cases[test_data.name]["pk"]}/',
            json=request_body
        )
        check_response_status(response)
...
def create_or_update_command(
    ...
    *,
    ...
    stitched_data = {
        ...
        'expected_stdout_instructor_file': get_instr_file(cmd_data.expected_stdout_instructor_file, instructor_files),
        'expected_stderr_instructor_file': get_instr_file(cmd_data.expected_stderr_instructor_file, instructor_files),
        'normal_fdbk_config': get_fdbk_conf(cmd_data.normal_fdbk_config),
        'first_failed_test_normal_fdbk_config': get_fdbk_conf(cmd_data.first_failed_test_normal_fdbk_config),
        'ultimate_submission_fdbk_config': get_fdbk_conf(cmd_data.ultimate_submission_fdbk_config),
        'past_limit_submission_fdbk_config': get_fdbk_conf(cmd_data.past_limit_submission_fdbk_config),
        'staff_viewer_fdbk_config': get_fdbk_conf(cmd_data.staff_viewer_fdbk_config),
    }

    print('    * Checking command', cmd_data.name, '...')

    request_body = cmd_data.model_dump(
        mode='json',
        exclude_unset=True,
